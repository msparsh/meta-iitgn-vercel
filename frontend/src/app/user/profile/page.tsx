          ...
          <div className="mx-auto max-w-6xl space-y-6">
            ...
            <section className="grid grid-cols-2 gap-3 sm:grid-cols-4">
              {[
                ...
                { label: "Articles improved", value: "28", icon: FileEdit, tone: "text-indigo-600" },
                { label: "Edits this month", value: "12", icon: TrendingUp, tone: "text-emerald-600" },
                { label: "Current streak", value: "5 days", icon: Flame, tone: "text-orange-500" },
            ...
            <div className="grid gap-6 pt-4 lg:grid-cols-[minmax(0,1.55fr)_minmax(280px,0.85fr)]">
              ...
              <aside className="space-y-6">
                <section className="rounded-3xl border border-base-300 bg-base-100 p-5 shadow-sm">
                  <div className="flex items-center justify-between"><h2 className="font-black text-base-content">Contribution goal</h2><Medal className="h-5 w-5 text-amber-500" /></div>
                ...
                <section className="rounded-3xl border border-base-300 bg-base-100 p-5 shadow-sm">
                  ...
                  <div className="mt-4 space-y-3 text-sm"><div className="flex items-center gap-3 text-base-content/65"><MapPin className="h-4 w-4 text-base-content/40" /> IIT Gandhinagar community</div><div className="flex items-center gap-3 text-base-content/65"><CalendarDays className="h-4 w-4 text-base-content/40" /> Joined {joined}</div><div className="flex items-center gap-3 text-base-content/65"><Heart className="h-4 w-4 text-base-content/40" /> {interests.length} areas of interest</div></div>
                </section>
              </aside>
            </div>

            <div className="grid gap-6 md:grid-cols-2">
              <section className="rounded-3xl border border-base-300 bg-base-100 p-5 shadow-sm sm:p-6"><div className="flex items-center justify-between"><div><h2 className="font-black text-base-content">Achievements</h2><p className="mt-1 text-sm text-base-content/55">Recognition earned through contribution.</p></div><Award className="h-5 w-5 text-amber-500" /></div><div className="mt-5 grid gap-3 sm:grid-cols-3">{badges.map(({ icon: Icon, title, description, tone }) => <div key={title} className="rounded-2xl bg-base-200/70 p-3"><span className={`flex h-9 w-9 items-center justify-center rounded-xl ${tone}`}><Icon className="h-4 w-4" /></span><p className="mt-3 text-sm font-bold text-base-content">{title}</p><p className="mt-1 text-xs leading-relaxed text-base-content/55">{description}</p></div>)}</div></section>
              <section className="rounded-3xl border border-base-300 bg-base-100 p-5 shadow-sm sm:p-6"><div className="flex items-center justify-between"><div><h2 className="font-black text-base-content">Your workspace</h2><p className="mt-1 text-sm text-base-content/55">Quick entry points for work in progress.</p></div><MoreHorizontal className="h-5 w-5 text-base-content/40" /></div><div className="mt-4 divide-y divide-base-200"><Link href="/user/contributions" className="flex items-center gap-3 py-3 text-sm font-semibold text-base-content hover:text-primary"><Clock3 className="h-4 w-4 text-primary" />Drafts and review queue <span className="ml-auto text-xs font-bold text-base-content/45">2 open</span></Link><Link href="/" className="flex items-center gap-3 py-3 text-sm font-semibold text-base-content hover:text-primary"><Bookmark className="h-4 w-4 text-primary" />Saved articles <span className="ml-auto text-xs font-bold text-base-content/45">6 saved</span></Link><Link href="/user/contributions" className="flex items-center gap-3 py-3 text-sm font-semibold text-base-content hover:text-primary"><UsersRound className="h-4 w-4 text-primary" />People you collaborate with <ChevronRight className="ml-auto h-4 w-4 text-base-content/40" /></Link></div></section>
            </div>
          </div>
        </main>
      </div>
